...
def check():
    ...
    if t:
        ...
        if now >= t:
            ...
            play_snd()
            # Выводим информационное окно с
            # текстом напоминания, и вопросом;
            # "выключить напоминание?"
            show_info = mb.askokcancel('Напоминание', f'{reminder_text} \nВыключить напоминание?')
            # Если пользователь дает положительный
            # ответ, тогда выключаем мелодию напоминания.
            if show_info:
                stop_music()
            # Присваиваем переменной t 0
            t = 0
